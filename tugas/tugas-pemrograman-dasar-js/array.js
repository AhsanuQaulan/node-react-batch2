console.log("SOAL 1");

var dataPeserta = ["john", "laki-laki", "programmer", "30"];

var output = "Halo, nama saya " + dataPeserta[0] + ". saya " + dataPeserta[1] + " berumur " + dataPeserta[3] + " bekerja sebagai seorang " + dataPeserta[2];

console.log(output);
console.log("");

console.log("SOAL 2");

var sayuran = [];

sayuran.push("Kangkung", "Bayam", "Buncis", "Kubis", "Timun", "Seledri", "Tauge");

console.log(sayuran);
console.log("");

console.log("SOAL 3");

var sayuran = ["Kangkung", "Bayam", "Buncis", "Kubis", "Timun", "Seledri", "Tauge"];

sayuran.sort();

console.log("Daftar Sayuran (diurutkan):");

for (var i = 0; i < sayuran.length; i++) {
  console.log(i + 1 + ". " + sayuran[i]);
}
console.log("");

console.log("SOAL 4");

var word = "car";
var output = [];

for (var i = 0; i < word.length; i++) {
  for (var j = i; j < word.length; j++) {
    output.push(word.slice(i, j + 1));
  }
}

console.log(output);
console.log("");

console.log("SOAL 5");

var numbers = [4, 5, 1, 4, 6, 8, 9, 21];
var total = 0;

for (var i = 0; i < numbers.length; i++) {
  total += numbers[i];
}

console.log(total);
console.log("");

console.log("SOAL 6");

var kumpulanAngka = [
  [1, 3, 5, 7, 8, 9],
  [4, 5, 6, 2, 3, 1],
  [6, 7, 8, 1, 3, 5],
];

var hasilPenjumlahan = kumpulanAngka.map(function (arrayBagian) {
  return arrayBagian.reduce(function (total, angka) {
    return total + angka;
  }, 0);
});

console.log(hasilPenjumlahan);
